...
import io.github.wasabithumb.xclaim.particle.legacy.LegacyParticleServiceReflection;
        ...
        if (!INIT) {
            try {
                if (PaperLib.isVersion(20)) {
                    if (PaperLib.isPaper()) {
                        INSTANCE = new io.github.wasabithumb.xclaim.particle.paper.PaperParticleService();
                    } else {
                        INSTANCE = new io.github.wasabithumb.xclaim.particle.spigot.SpigotParticleService();
                    }
                } else {
                    LegacyParticleServiceReflection reflect = loadParticleLib();
                    INSTANCE = new io.github.wasabithumb.xclaim.particle.legacy.LegacyParticleService(reflect);
                }
            } catch (Exception e) {
