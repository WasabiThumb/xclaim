...
public class SpongePlatformBlockPlaceEvent
        extends SpongePlatformEvent<ChangeBlockEvent.All>
        implements PlatformBlockPlaceEvent
{
    ...
    public static void adapt(
        ...
        for (BlockTransaction bt : handle.transactions()) {
            if (!RegistryUtil.referenceEquals(RegistryTypes.OPERATION, bt.operation(), Operations.PLACE)) continue;
            queue.add(new SpongePlatformBlockPlaceEvent(platform, handle, bt));
        }
    }
    ...
    private SpongePlatformBlockPlaceEvent(
            @NotNull SpongePlatform platform,
            @NotNull ChangeBlockEvent.All handle,
